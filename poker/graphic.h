...
int pickCardof_Hard_Dealer(int *cardrest, int **hand)
{
    ...
    if (oldSta == 8)
    { // if Four of a kind -> place the other
        for (int i = 0; i < 5; i++)
        {
    ...
    if (getStatusOfHand(Dealer) >= getStatusOfHand(hand))
    {
        for (int i = 0; i < 5; i++)
        {
            for (int j = 0; j < 5; j++)
            {
                if (hand[i][0] != Dealer[j][0] || hand[i][1] != Dealer[j][1])
                    theSAME++;
            }
            if (theSAME == 5)
                return i;
...
bool dealerChoseHard(int **dealer, int *tempCard, int times)
{
    ...
    while (true)
    {
        ...
        //yes =0   || no =1
        ///
        if (pickCardof_Hard_Dealer(tempCard, dealer) > -1)
            chose = 0;
        else
            chose = 1;
